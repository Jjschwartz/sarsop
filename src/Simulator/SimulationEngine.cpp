...
namespace momdp
{
  ...
  void SimulationEngine::printCurrentBeliefVec(belief_vector& b, ostream& streamOut)
  {
    streamOut << "Belief :" << endl;
    for(unsigned int i = 0; i < b.filled(); i++)
    {
      streamOut << "\t(";
      map<string, string> tuple = problem->getFactoredUnobservedStatesSymbols(b.data[i].index);
      for(map<string, string>::iterator iter = tuple.begin() ; iter != tuple.end() ; )
      {
        streamOut << iter->second;
        if(++iter!=tuple.end())
  ...
  int SimulationEngine::runFor(int iters, ofstream* streamOut, double& reward, double& expReward)
  {
    ...
    for(int timeIter = 0; timeIter < iters; timeIter++)
    {
      ...
      {
          //cout<<" BHOUT "<<te!! timeIter " << timeIter << endl;
          if(enableFiling)bhout[ii];
      }
      ...
      if(enableFiling)
      {
        ...
        if (timeIter == 0)
        {
          ...
          printTuple(unobsState, streamOut);	string md5HashValue();
